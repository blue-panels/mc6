...
клавиши
**F9**
или щелчка мыши по верхней рамке экрана. Меню состоит из шести пунктов:
"Левая", "Файл", "Атрибуты", Команды", Настройки" и "Правая". При выборе одного
...
Меню
["Атрибуты"](#attributes-menu)
собирает команды, которые меняют права доступа, владельца и флаги файловой
системы тех же файлов.
...
## Меню 'Атрибуты' <a id="attributes-menu"></a>

Команды этого меню меняют не содержимое файла, а то, что о нём знает файловая
система: права доступа, владельца и группу, флаги файловой системы. Каждая из
них работает с выделенным файлом или с группой помеченных файлов.

**Права доступа... (C-x c)**
: Изменить права доступа в диалоговом окне
[Права доступа](#chmod).

**Владелец/группа... (C-x o)**
: Изменить владельца и группу в диалоговом окне
[Владелец/группа](#chown).

**Права (расширенные)...**
: Изменить права доступа, владельца и группу в одном диалоговом окне, смотрите
[Расширенная команда Chown](#advanced-chown).

**Флаги chattr... (C-x e)**
: Изменить флаги файловой системы ext2, ext3 или ext4 в диалоговом окне
[Команда Chattr](#chattr). Пункт есть только в сборке с поддержкой этих флагов.
...
Пункт меню
**Править файл расцветки имён**
открывает файл, который задаёт, какие имена и какие типы файлов панель
показывает каким цветом, смотрите
...
значения не имеет. Поиск идёт вниз от курсора и отмечает ближайшее
совпадение; Alt-s ещё раз отмечает следующее ниже, а за самой новой строкой
поиск переходит к самой старой. Там, где печатает оболочка, ещё ничего не
читали, и поиск берёт выдачу с самой старой строки. Фильтр показывает только
совпавшие строки, и клавиши управления курсором ходят по ним прямо во время
набора; Alt-Shift-s ещё раз переводит курсор на строку ниже. Настройка
*search_direction*
разворачивает обе клавиши: поиск идёт вверх по выдаче, как в
**less**
и как было раньше. Нажатые без набранного
...
набор и оставляет обзор на найденном, вместе с отметкой; Esc заканчивает
его и возвращает обзор к тому, что было до набора: курсор туда, где читали,
или на промпт, если не читали, и фильтр с отметкой, какие были. Любая
другая клавиша заканчивает набор и делает то, что делает.
...
Настройки терминала, в котором за панелями работает оболочка, находятся в
секции [Terminal] того же файла. Диалога у них нет.

*search_direction*
: В какую сторону
**Alt-s**
просматривает выдачу оболочки и в какую
**Alt-Shift-s**
переходит от одной строки фильтра к следующей: "down" - от курсора к самой
новой строке и за ней по кругу к самой старой, "up" - к самой старой и по
кругу к самой новой, как ищет
**less**
и как было раньше. По умолчанию "down".
