...
typedef struct
{
    ...
    sqlite3 *db;
    char *db_path;
    /* Set when the bytes came from another panel: the name to show, while
       db_path is only a temporary copy nobody can navigate back to. */
    char *display_name;
    /* Held for as long as the database is open, because the file behind it is
       the one sqlite3 reads. */
    mc_pp_input_stream_t *input_stream;
    /* Our own copy of a stream that had no file; removed with the panel. */
    char *temp_path;
    char *object_name;
    char *title;
    char *focus_after_up;
...
static void *sqlite_open_input_stream (mc_panel_host_t *host, const char *display_name,
                                       mc_pp_input_stream_t *stream);
static mc_pp_result_t sqlite_view_input_stream (mc_panel_host_t *host, const char *display_name,
                                                mc_pp_input_stream_t *stream, char **local_path);
static gboolean sqlite_may_open_name (const char *display_name);
static gboolean sqlite_write_all (int fd, const char *text, gsize length);
...
static const mc_pp_file_operation_t sqlite_file_operations[] = {
    {
        .name = "open",
        .kind = MC_PP_FILE_OPERATION_OPEN,
        .may_open_name = sqlite_may_open_name,
        .open_input_stream = sqlite_open_input_stream,
    },
    {
        .name = "view",
        .kind = MC_PP_FILE_OPERATION_VIEW,
        .may_open_name = sqlite_may_open_name,
        .view_input_stream = sqlite_view_input_stream,
    },
};
...
static const mc_panel_plugin_t sqlite_plugin = {
    ...
    .open_input_stream = sqlite_open_input_stream,
    .file_operations = sqlite_file_operations,
    .file_operation_count = G_N_ELEMENTS (sqlite_file_operations),
    .chdir = sqlite_chdir,
...
/* The database as the panel names it: the file itself for a local path, and
   the entry it was read from when the bytes came from another panel. */
static char *
sqlite_display_base (sqlite_data_t *data)
{
    if (data->display_name != NULL)
        return g_strdup (data->display_name);

    return g_path_get_basename (data->db_path);
}
...
sqlite_focus_database_file (sqlite_data_t *data)
{
    if (data->host != NULL)
    {
        ...
        data->host->focus_after = sqlite_display_base (data);
    }
}
...
sqlite_object_rowid_name (sqlite_data_t *data, const char *name)
{
    ...
    /* Take a rowid alias no column of this table shadows. */
...
sqlite_get_location (void *plugin_data)
{
    ...
    /* A temporary copy is gone by the time anyone follows the location. */
    if (data->display_name != NULL)
        return NULL;
...
/* @stream_name is set only when @path is a copy made for this panel: the panel
   then shows that name and hands back no location. */
static sqlite_data_t *
sqlite_open_path (mc_panel_host_t *host, const char *path, const char *navigation,
                  gboolean report_errors, const char *stream_name)
{
    sqlite_data_t *data;
    char *canonical;
    char *error = NULL;
    ...
    data->display_name = g_strdup (stream_name);
    data->level = SQLITE_LEVEL_OBJECTS;
...
/* Only for a panel: the history is about where the user is, and reading a
   database out for the viewer takes them nowhere. */
static sqlite_data_t *
sqlite_open_panel_path (mc_panel_host_t *host, const char *path, const char *navigation,
                        gboolean report_errors)
{
    sqlite_data_t *data;

    data = sqlite_open_path (host, path, navigation, report_errors, NULL);
    if (data != NULL)
...
sqlite_open (mc_panel_host_t *host, const char *open_path)
{
    if (open_path != NULL && g_str_has_prefix (open_path, sqlite_plugin.prefix))
    {
        ...
        data = sqlite_open_panel_path (host, db_path, navigation, TRUE);
        g_free (db_path);
        g_free (navigation);
        return data;
    }
    ...
    if (open_path != NULL && g_file_test (open_path, G_FILE_TEST_IS_REGULAR))
        return sqlite_open_panel_path (host, open_path, NULL, TRUE);

    {
        ...
        if (path != NULL && *path != '\0')
        {
            sqlite_data_t *data = sqlite_open_panel_path (host, path, NULL, TRUE);
...
/* Only a name gate: it decides whether a file is worth fetching out of another
   panel, and the header check on the bytes has the last word. */
static gboolean
sqlite_may_open_name (const char *display_name)
{
    static const char *const extensions[] = {
        ".sqlite", ".sqlite3", ".sqlitedb", ".db", ".db3", NULL,
    };
    const char *base;
    size_t base_len;
    int i;
    ...
    base = strrchr (display_name, '/');
    base = base != NULL ? base + 1 : display_name;
    ...
    for (i = 0; extensions[i] != NULL; i++)
    {
        ...
        if (base_len > len && g_ascii_strcasecmp (base + base_len - len, extensions[i]) == 0)
            return TRUE;
    }
...
/* sqlite3 reads a file it can seek in, so a stream without one is written out
   once.  The copy is the panel's and goes away with it. */
static gboolean
sqlite_stream_to_temp_file (mc_pp_input_stream_t *stream, char **temp_path)
{
    GError *error = NULL;
    void *handle = NULL;
    int fd;
    ...
    *temp_path = NULL;
    ...
    fd = g_file_open_tmp ("mc-sqlite-db-XXXXXX", temp_path, &error);
    if (fd == -1)
    {
        g_clear_error (&error);
    ...
    while (ok)
    {
        ...
        got = stream->ops->read (stream, handle, buffer, sizeof (buffer), &error);
        if (got < 0)
        {
            g_clear_error (&error);
            ok = FALSE;
            ...
            ok = sqlite_write_all (fd, buffer, (gsize) got);
    }
...
/* The stream is kept for as long as the database is open: the file sqlite3
   reads is the one behind it. */
static sqlite_data_t *
sqlite_open_stream (mc_panel_host_t *host, const char *display_name, mc_pp_input_stream_t *stream)
{
    sqlite_data_t *data;
    const char *path;
    char *temp_path = NULL;
    ...
    if (stream == NULL || display_name == NULL)
        return NULL;

    path = mc_pp_input_stream_local_path (stream, &temporary);
    if (path == NULL)
    {
        if (!sqlite_stream_to_temp_file (stream, &temp_path))
        {
            sqlite_show_error (host, _ ("Cannot read the database."));
            return NULL;
        }
        path = temp_path;
        temporary = TRUE;
    ...
    data = sqlite_open_path (host, path, NULL, TRUE, temporary ? display_name : NULL);
    if (data == NULL)
    {
        if (temp_path != NULL)
        {
...
/* magic.ini's Open operation. */
static void *
sqlite_open_input_stream (mc_panel_host_t *host, const char *display_name,
                          mc_pp_input_stream_t *stream)
{
    sqlite_data_t *data;

    data = sqlite_open_stream (host, display_name, stream);
    if (data != NULL)
        sqlite_remember_location (data);
...
/* magic.ini's View operation: F3 on a database file shows its schema.  No
   panel and no dialog here; the core owns the viewer and its temporary file. */
static mc_pp_result_t
sqlite_view_input_stream (mc_panel_host_t *host, const char *display_name,
                          mc_pp_input_stream_t *stream, char **local_path)
{
    sqlite_data_t *data;
    char *content;
    ...
    *local_path = NULL;
    ...
    if (content != NULL)
    {
        result = sqlite_content_to_local_copy (content, "schema.sql", local_path);
        g_free (content);
    }
    else
        result = MC_PPR_FAILED;

    /* The caller keeps a stream that produced nothing to view. */
    if (result != MC_PPR_OK)
