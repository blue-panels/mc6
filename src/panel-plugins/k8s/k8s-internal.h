...
#define K8S_NS_FAVS_CONFIG_GROUP        "k8s-ns-favs"
...
#define K8S_PANEL_KEY_LOGS_OPTS         "hotkey_logs_options"
...
typedef struct
{
    ...
    char *restarts; /* pods */
    char *age;
    char *restage;   /* pods: "<restarts>/<age>" combined */
    char *node_name; /* pods: 4-char suffix (after last '-') */
    char *ready;     /* pods: "1/2"; deployments: "2/2" */
    char *ip;        /* pods: short "a.b" (last two octets) */
    char *image;     /* pods: first container's image (full) */
    char *namespace; /* pods: source namespace (for all-namespaces actions) */
...
typedef struct
{
    ...
    char *selected_pod;
    char *selected_namespace; /* namespace of selected_pod (for all-namespaces) */
    char *pending_focus;
    ...
    GHashTable *item_index;  /* item->name -> k8s_item_t* (borrowed); O(1) find_item */
    gpointer index_for;      /* the items array item_index was built for; NULL = stale */
    gboolean ns_show_all;    /* TRUE: namespaces view bypasses favs filter */
    gboolean all_namespaces; /* TRUE: resource queries use --all-namespaces */
} k8s_data_t;
...
extern const char k8s_ns_all_entry[];
...
GPtrArray *k8s_ns_favs_load (const char *context);
void k8s_ns_favs_save (const char *context, GPtrArray *favs);
...
const char *k8s_deployments_get_column_value (k8s_data_t *data, const char *name, const char *col);
