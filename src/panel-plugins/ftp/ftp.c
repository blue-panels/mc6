...
ftp_load_hotkey (const char *key, const char *fallback_text, int fallback_key)
{
    ...
    hotkey = mc_plugin_prefs_load_hotkey (FTP_PANEL_CONFIG_FILE, FTP_PANEL_CONFIG_GROUP, key,
...
ftp_connection_to_local_copy (const ftp_connection_t *conn, char **local_path)
{
    ...
    ok = mc_pp_write_temp_file ("mc-ftp-view-XXXXXX", ini->str, (gssize) ini->len, &tmp_path);
...
static mc_pp_result_t
ftp_clone_connection (ftp_data_t *data)
{
    const GString *current_name;
    ftp_connection_t *src, *clone;
...
ftp_handle_key (void *plugin_data, int key)
{
    ...
    {
        if (data->at_root)
            return ftp_clone_connection (data);
        return MC_PPR_NOT_SUPPORTED;
    }
