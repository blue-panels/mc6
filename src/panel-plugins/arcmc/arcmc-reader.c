...
static la_int64_t
arcmc_archive_reader_stream_seek (struct archive *archive, void *client_data, la_int64_t offset,
                                  int whence)
{
    arcmc_archive_reader_ctx_t *ctx = (arcmc_archive_reader_ctx_t *) client_data;
    GError *error = NULL;
    ...
    position = ctx->stream->ops->seek (ctx->stream, ctx->handle, offset, whence, &error);
    if (position < 0)
    {
        arcmc_archive_reader_set_error (archive, error, "Cannot seek archive input stream");
        return ARCHIVE_FATAL;
    }
...
arcmc_archive_reader_stream_skip (struct archive *archive, void *client_data, la_int64_t request)
{
    ...
    if (ctx->stream->ops->seek != NULL)
        return arcmc_archive_reader_stream_seek (archive, client_data, request, SEEK_CUR) < 0
            ? -1
            : request;
...
arcmc_archive_reader_open (const arcmc_data_t *data, arcmc_archive_reader_ctx_t **ctx)
{
    ...
    *ctx = NULL;
    ...
    else
    {
        ...
        archive_read_set_callback_data (archive, stream_ctx);
        archive_read_set_open_callback (archive, arcmc_archive_reader_stream_open);
        archive_read_set_read_callback (archive, arcmc_archive_reader_stream_read);
        archive_read_set_skip_callback (archive, arcmc_archive_reader_stream_skip);
        archive_read_set_close_callback (archive, arcmc_archive_reader_stream_close);
        /* 7z and other formats with a trailing directory are only readable when
           the source can be repositioned. */
        if (data->input_stream->ops != NULL && data->input_stream->ops->seek != NULL)
            archive_read_set_seek_callback (archive, arcmc_archive_reader_stream_seek);

        result = archive_read_open1 (archive);
    }
