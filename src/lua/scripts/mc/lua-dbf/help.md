...
F3 or Enter on a dBase / FoxPro / Clipper table opens it full screen:
one row per record, one column per field, the first column the
...
```
  ...
  Left, Right - scroll the columns when they do not fit;
               Ctrl-Left, Ctrl-Right scroll them by half a screen
  Enter, F3  - the record as a card, one field per line
  F2         - the structure of the table: the header and the
               field list
  F4         - hide or show the deleted records
  F5         - the encoding: auto (by the language driver byte of
               the header), cp866, cp1251, cp1252 or UTF-8
