...
broken, a table or a schema, is scrolled sideways instead.
...
The dialog is opened with "i" in the viewer, and from the Options menu
...
```
  ...
  Frame a code block
               Draw the corners of a block of code and write the
               language of the fence in the top edge.

  Background of a code block
               A shade of the background the skin paints the viewer
               with, needs a terminal of 256 colors or more.  None
               leaves the background of the terminal untouched.  The
               third choice takes the SGR written under it, such as
               "100" or "48;5;236".

  How far that shade moves
               How far the shade is moved away from the background of
               the viewer, of 255 per channel.

  The narrowest a code block gets
               A short block is drawn no narrower than this, so that a
               line of two words is not a stripe.

  A decision in a diagram
               The sloping sides of a decision are drawn with braille
               dots, or with box drawing characters for a font that has
               no braille.

  A diagram is laid out up to this width
...
not know is lost; everything the dialog does not hold is left in
config.lua of the plugin.
